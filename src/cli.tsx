	...
	.action(async (ref, options) => {
		try {
			const gitCommand = (() => {
				...
				if (options.commit)
					return `git show ${options.commit} --no-prefix`;
			...
			function AppWithWatch() {
				...
				React.useEffect(() => {
					if (parsedFiles && parsedFiles.length > 0) {
						const currentIndex =
							useDiffStore.getState().currentFileIndex;
...
cli.command("difftool <local> <remote>", "Git difftool integration").action(
...
cli.command(
	"pick <branch>",
	"Pick files from another branch to apply to HEAD",
			...
			const handleChange = async (value: string) => {
				...
				if (isSelected) {
					...
					usePickStore.setState((state) => ({
						selectedFiles: new Set(
							Array.from(state.selectedFiles).filter(
								(f) => f !== value,
							),
						),
						appliedFiles: new Map(
							Array.from(state.appliedFiles).filter(
								([k]) => k !== value,
							),
						),
					}));
					...
					if (result1.error) {
						const result2 = execSyncWithError(
							`git apply "${patchFile}"`,
							{
								stdio: "pipe",
							},
...
cli.command("web [ref]", "Generate web preview of diff")
	.option("--staged", "Show staged changes")
	.option("--commit <ref>", "Show changes from a specific commit")
	.option(
		"--cols <cols>",
	...
	.action(async (ref, options) => {
		...
		const diffFile = join(
			tmpdir(),
			`critique-web-diff-${Date.now()}.patch`,
		);
...
cli.command("web-render <diffFile>", "Internal: Render diff for web capture", {
