...
import { buildReviewModeScript, REVIEW_MODE_CSS } from "./review/web-review-mode.js"
...
export async function captureToHtml(
  ...
  const renderLineCallback = (defaultHtml: string, line: CapturedLine, lineIndex: number) => {
    ...
    while (sectionPtr < sortedSections.length && lineIndex >= (sortedSections[sectionPtr]?.lineIndex ?? Number.POSITIVE_INFINITY)) {
      currentFile = sortedSections[sectionPtr]?.fileName ?? null
      sectionPtr++
    }
